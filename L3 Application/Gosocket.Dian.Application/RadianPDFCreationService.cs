...
namespace Gosocket.Dian.Application
{
    public class RadianPdfCreationService : IRadianPdfCreationService
    {
        ...
        public async Task<byte[]> GetElectronicInvoicePdf(string eventItemIdentifier, string webPath)
        {
            ...
            StringBuilder templateFirstPage = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistencia.html"));
            StringBuilder templateLastPage = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistenciaFinal.html"));
            StringBuilder footerTemplate = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistenciaFooter.html"));
            StringBuilder firstEvent = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistenciaFirstEvent.html"));
            ...
            if (events.Any())
            {
                for (int i = 1; i < events.Count; i++)
                {
                    ...
                    {
                        page++;
                        middleTemplate = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistenciaHeader.html"));
                        eventTemplate = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistenciaInterna.html"));
            ...
            {
                page++;
                headerTemplate = new StringBuilder(RadianTemplatesFileManager.GetText( "CertificadoExistenciaHeader.html"));
                templateLastPage = templateLastPage.Replace("{DocumentsTotal}", documents.Count.ToString());
                templateLastPage = templateLastPage.Replace("{EventsTotal}", eventCount.ToString());
                templateLastPage = templateLastPage.Replace("{ExpeditionDate}", expeditionDate.ToShortDateString());
                templateLastPage = templateLastPage.Replace("{QRCode}", ImgHtml);
        ...
        {
            byte[] bytesLogo = RadianLogosFileManager.GetBytes( "Logo-DIAN-2020-color.jpg");
            byte[] bytesFooter = RadianLogosFileManager.GetBytes("GroupFooter.png");
