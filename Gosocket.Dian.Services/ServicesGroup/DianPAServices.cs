...
namespace Gosocket.Dian.Services.ServicesGroup
{
    public class DianPAServices : IDisposable
    {
        ...
        private static readonly FileManager GlobalFileManager = new FileManager(blobContainer);
        ...
        public async Task<UploadDocumentResponse> ProcessBatchZipFile(string fileName, byte[] contentFile, string authCode = null, string testSetId = null)
        {
            ...
            var result = await GlobalFileManager.UploadAsync($"{blobContainerFolder}/{blobPath}/{zipKey}.zip", contentFile);
            if (!result)
            {
        ...
        public async Task <ExchangeEmailResponse> GetExchangeEmails(string authCode)
        {
            ...
            var bytes = await DianFileManager.GetBytesAsync( $"exchange/emails.csv");
        ...
        public async Task<List<DianResponse>> GetBatchStatus(string trackId)
        {
            ...
            if (exist)
            {
                var zipBytes = await GlobalFileManager.GetBytesAsync($"{blobContainerFolder}/applicationResponses/{trackId}.zip");
                if (zipBytes != null)
                {
