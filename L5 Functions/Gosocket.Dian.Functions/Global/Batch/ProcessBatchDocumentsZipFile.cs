...
namespace Gosocket.Dian.Functions.Global.Batch
{
    public static class ProcessBatchDocumentsZipFile
    {
        ...
        public static async Task Run([QueueTrigger("global-process-batch-zip-input", Connection = "GlobalStorage")]string myQueueItem, TraceWriter log)
        {
            ...
            try
            {
                ...
                //Get multiple xpath values
                log.Info($"init get xpath values xml큦.");
                var multipleResponsesXpathDataValue = await ApiHelpers.ExecuteRequestAsync<List<ResponseXpathDataValue>>(ConfigurationManager.GetValue("GetMultipleXpathDataValuesUrl"), requestObjects);
                log.Info($"End get xpath values xml큦.");
                // filer by success
