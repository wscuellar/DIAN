			...
			try
			{
				...
				if (objNomina.TipoXML == "102")
				{
					...
					var InsertDocumentDataPayroll = new Domain.Cosmos.DocumentData()
					{
						...
						GenerationDate = objNomina.FechaGen == null ? DateTime.Parse(objNomina.FechaGenPred.ToString()).ToString("yyyy-MM-dd") : DateTime.Parse(objNomina.FechaGen.ToString()).ToString("yyyy-MM-dd"),
						GenerationDateNumber = objNomina.FechaGen == null ? DateTime.Parse(objNomina.FechaGenPred.ToString()).ToString("yyyy-MM-dd") : DateTime.Parse(objNomina.FechaGen.ToString()).ToString("yyyy-MM-dd"),
